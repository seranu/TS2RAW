...
#define TEST_END(res) if(res) { cout << "FAILED\n"; } else { cout << "PASSED\n"; }\
                       return res;
#define ASSERT_TRUE(cond) if (!(cond)) { cout << "Unit test " << __FUNCTION__ << " failed at line: " << __LINE__ << "\n"; TEST_END(1); }
...
int testUtils_01_ReadUInt32_BE_01() {
    TEST_START
    const unsigned char buffer[] = { 0x01, 0x11, 0xFF, 0xAB, 0xCB, 0xBD, 0x9A, 0xF1 };
...
std::pair<int, int> runTests(const std::vector<std::function<int()>> & aTestSuite) {
	int failed = 0;
	int passed = 0;
